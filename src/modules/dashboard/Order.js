              ...
              <tbody>
                {orders.map((order, index) => (
                  <tr key={order._id}>
                    ...
                    <td>{order.cartItems.map(item => item.price)}</td>
                    <td style={{color:'yellow'}}>{order.cartItems.map(item => item.quantity)}</td>
                    ...
                    <td>
                      ...
                      <button className="btn btn-success btn-sm ms-1" onClick={() => handleStatusChange(order._id, 'Delivered')}>
                      Delivered
                      </button>
